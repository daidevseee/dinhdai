...
const startBot = function(values) {
  console.log(`${chalk.bgBlue("[INFO]")} ${chalk.blue("Đang khởi động bot ... quá trình này có thể mất vài giây để bắt đầu ...")}`);
  ...
  var shutdown = function() {
    console.log(`${chalk.bgRed(`[STATUS]`)} ${chalk.red('Đang Tắt')}`);
...
if (process.argv.includes('-c') || process.argv.includes('--config')) {
  setup.loadValues().then((values) => {
    ...
    console.log('Không thể tải các giá trị đã lưu, hãy định cấu hình lại tất cả các giá trị đã lưu');
  ...
  console.log(`${chalk.bold.bgYellow(`[LOAD]`)} ${chalk.bold.yellow('Đang cố tải các giá trị môi trường ...')}`);
  setup.loadValues().then((values) => {
    startBot(values);
  }).catch((error) => {
    console.error(error);
